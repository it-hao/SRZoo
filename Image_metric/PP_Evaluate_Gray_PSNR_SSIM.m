function PP_Evaluate_Gray_PSNR_SSIM()
...
noise_all = [10, 20, 30, 40];

for idx_method = 1:num_method
    record_results_txt = ['PSNR_SSIM_Results_', methods{idx_method}, '.txt'];
    ...
    for idx_set = 1:num_set
        ...
        for noise = noise_all
            ...
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile('./HQ', dataset{idx_set}, ['Q', num2str(noise)], ext{idx_ext})));
            end
            PSNR_all = zeros(1, length(filepaths));
            SSIM_all = zeros(1, length(filepaths));
            for idx_im = 1:length(filepaths)
                name_HQ = filepaths(idx_im).name;
                name_SQ = strrep(name_HQ, 'HQ', methods{idx_method});
                im_HQ = imread(fullfile('./HQ', dataset{idx_set}, ['Q', num2str(noise)], name_HQ));
                im_SQ = imread(fullfile('./', [methods{idx_method}], dataset{idx_set}, ['Q', num2str(noise)], [name_SQ(1:end-4), '.jpg']));
                % im_SQ = imread(fullfile('./', [methods{idx_method}], dataset{idx_set}, ['Q', num2str(noise)], name_SQ));
                ...
                PSNR_all(idx_im) = csnr(im_HQ, im_SQ, 0, 0);
                [~, SSIM_all(idx_im)] = Cal_PSNRSSIM(im_HQ, im_SQ, 0, 0);
                
                fprintf(results, 'x%d %d %s: PSNR= %f SSIM= %f\n', noise, idx_im, name_SQ, PSNR_all(idx_im), SSIM_all(idx_im));
            end
            fprintf(results, '--------Mean--------\n');
            fprintf('--------Mean--------\n');
            fprintf(results, 'x%d: PSNR= %f SSIM= %f\n', noise, mean(PSNR_all), mean(SSIM_all));
